...
class data():
    ...
    def getAlbums(content_category):
        sql_data = 'SELECT a.id,a.permalink,a.title,a.story,a.genre,a.album_type,a.content_category,a.date_added,s.id as song_id,s.song_file_name,s.poster,s.duration,s.is_episode FROM album a,song s WHERE a.id=s.album_id AND a.album_status=1 AND s.is_episode = 0 AND FIND_IN_SET({}, a.content_category)'.format(content_category)
        res = Album.objects.raw(sql_data)
        return res
    def getSearchSong(searchKey):
        sql_data = 'SELECT a.id,a.permalink,a.title,a.story,a.genre,a.album_type,a.content_category,a.date_added,s.id as song_id,s.song_file_name,s.poster,s.duration,s.is_episode,s.name FROM album a,song s WHERE a.id=s.album_id AND (a.title like "%%'+str(searchKey)+'%%" OR s.name like "%%'+str(searchKey)+'%%")'
        res = Album.objects.raw(sql_data)
        return res    
    def getSongs():
        songDetails = {}
        data = []
        sql_data = 'SELECT a.id,a.permalink,a.title,a.story,a.genre,a.album_type,a.content_category,a.date_added,s.id as song_id,s.song_file_name,s.poster,s.duration,s.is_episode FROM album a, song s WHERE a.id=s.album_id'
        res = Album.objects.raw(sql_data)
        for i in res:
            songData = {
                "id" : i.id,
    ...
    def getSingle():
        songDetails = {}
        data = []
        sql_data = 'SELECT a.id,a.permalink,a.title,a.story,a.genre,a.album_type,a.content_category,a.date_added,s.id as song_id,s.song_file_name,s.poster,s.duration,s.is_episode FROM album a, song s WHERE a.id=s.album_id AND a.album_type=0 ORDER BY date_added DESC LIMIT 10';
        res = Album.objects.raw(sql_data)
        for i in res:
            songData = {
                "id" : i.id,
    ...
    def checkUser(request, email, password):
        try:
            validUser = User.objects.get(email = str(email), password = str(password))
            request.session['userid'] = validUser.id
    ...
    def checkUserExist(email):
        try:
            checkUser = User.objects.get(email = str(email))  
        except:
            checkUser = []
        return checkUser      
